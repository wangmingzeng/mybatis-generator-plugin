...
public class SpringServiceInterfacePlugin extends PluginAdapter {
	...
	public boolean validate(List<String> warnings) {
		...
		serviceInterfaceTargetPackage = prop.getProperty("serviceInterfaceTargetPackage");
		boolean valid2 = stringHasValue(serviceInterfaceTargetPackage);

		serviceImplTargetPackage = prop.getProperty("serviceImplTargetPackage");
		...
		for (GeneratedJavaFile javaFile : introspectedTable.getGeneratedJavaFiles()) {
			...
			if (shortName.endsWith("Mapper")) {/// 复制Mapper代码创建Spring Service 接口
				try {
					...
					springServiceImplJavaFile = generateSpringServiceImplFile(baseModelJavaType, javaFile,
							springServiceJavafile);
	...
	public boolean checkFileIsExist(String targetDir, String targetPackage, GeneratedJavaFile file)
	...
	public GeneratedJavaFile generateSpringServiceImplFile(FullyQualifiedJavaType baseModelJavaType,
			GeneratedJavaFile javaFile, GeneratedJavaFile springServiceJavafile) {
		JavaFormatter javaFormatter = context.getJavaFormatter();
	...
	public void addImportType(TopLevelClass topLevelClass, FullyQualifiedJavaType baseModelJavaType,
			GeneratedJavaFile springServiceJavafile) {
	...
	public void addMethod(TopLevelClass topLevelClass, GeneratedJavaFile javaFile, String shortName) {
		...
		for (Method method : ((Interface) javaFile.getCompilationUnit()).getMethods()) {
			...
			StringBuffer bodyLine = new StringBuffer("return ").append(mapperVariableName).append(".")
